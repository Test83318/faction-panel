...
const AccountSettings: React.FC = () => {
    ...
    const [scrollingText, setScrollingText] = useState(() => localStorage.getItem('roster-scrolling-text') === 'true');
    ...
    const toggleScrollingText = () => {
        const newValue = !scrollingText;
        setScrollingText(newValue);
        localStorage.setItem('roster-scrolling-text', newValue.toString());
        toast.success('Preference updated');
    };
                    ...
                    <div className="space-y-8">
                        ...
                        <section className="bg-card border border-border rounded-2xl p-6 shadow-sm">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex items-center justify-between p-4 bg-surface border border-border rounded-xl group/toggle">
                                    <div className="flex flex-col">
                                        <span className="text-xs font-bold uppercase tracking-tight">Scrolling Text</span>
                                        <span className="text-[8px] text-muted uppercase tracking-widest font-medium">Scroll long cell text on hover instead of scaling it down</span>
                                    </div>
                                    <button
                                        onClick={toggleScrollingText}
                                        className={`w-10 h-5 rounded-full relative transition-all duration-300 shadow-inner shrink-0 ${scrollingText ? 'bg-accent' : 'bg-muted/20'}`}
                                    >
                                        <div className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-all duration-300 shadow-md ${scrollingText ? 'right-1' : 'left-1'}`} />
                                    </button>
                                </div>
                            </div>
                        </section>
